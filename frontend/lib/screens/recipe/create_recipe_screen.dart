...
class _CreateRecipeScreenState extends State<CreateRecipeScreen> {
  ...
  void fillForm() {
    ...
    for (final group in recipe.ingredientGroups) {
      ingredientGroups.add(LocalIngredientGroup(name: group['name'] ?? '', ingredients: []));
    ...
    for (final stepData in recipe.steps) {
      steps.add(
        LocalStep(
          ...
          imageUrls: (stepData['images'] as List).map<String>((image) => image['image_url']).toList(),
        ),
      );
    }
    setState(() {});
        ...
        child: Column(
          ...
          children: [
            ...
            Container(
              ...
              child: Column(
                children: [
                  _buildTextField(titleController, 'Judul Resep', icon: Icons.restaurant_menu),
                  const SizedBox(height: 16),
                  _buildTextField(descriptionController, 'Deskripsi', maxLines: 3),
                  const SizedBox(height: 16),
                  ...
                  Row(
                    children: [
                      Expanded(child: _buildTextField(cookTimeController, 'Waktu (Menit)', isNumber: true, icon: Icons.timer_outlined)),
                      const SizedBox(width: 12),
                      Expanded(child: _buildTextField(servingsController, 'Porsi', isNumber: true, icon: Icons.people_outline)),
                    ],
                  ),
                  const SizedBox(height: 16),
                  _buildTextField(costController, 'Estimasi Biaya (Rp)', isNumber: true, icon: Icons.monetization_on_outlined),
                ],
              ),
            ),
            const SizedBox(height: 24),
            ...
            Container(
              ...
              child: Column(
                children: [
                  SwitchListTile(
                    title: const Text('Mengandung Babi', style: TextStyle(fontWeight: FontWeight.bold)),
                    subtitle: const Text('Aktifkan jika resep menggunakan bahan daging/minyak babi'),
                    value: containsPork,
                  ...
                  SwitchListTile(
                    title: const Text('Mengandung Alkohol', style: TextStyle(fontWeight: FontWeight.bold)),
                    subtitle: const Text('Aktifkan jika resep menggunakan arak masak, mirin, dsb'),
                    value: containsAlcohol,
                    activeColor: Colors.red,
                    secondary: const Icon(Icons.liquor_outlined, color: Colors.red),
            ...
            const SizedBox(height: 12),
            ...ingredientGroups.asMap().entries.map((entry) => _buildIngredientGroup(entry.key, entry.value)).toList(),
            const SizedBox(height: 12),
            Center(
            ...
            const SizedBox(height: 12),
            ...steps.asMap().entries.map((entry) => _buildStepCard(entry.key, entry.value)).toList(),
            const SizedBox(height: 12),
            Center(
              child: TextButton.icon(
                onPressed: addStep,
  ...
  Widget _buildTextField(TextEditingController controller, String label, {int maxLines = 1, bool isNumber = false, IconData? icon}) {
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        child: coverImage != null
            ? ClipRRect(borderRadius: BorderRadius.circular(14), child: Image.file(coverImage!, fit: BoxFit.cover))
            : existingCoverImage != null
                ? ClipRRect(borderRadius: BorderRadius.circular(14), child: Image.network(ApiConstants.baseUrl + existingCoverImage!, fit: BoxFit.cover))
                : Column(
                    mainAxisAlignment: MainAxisAlignment.center,
                    children: [
                      Icon(Icons.add_a_photo_outlined, size: 48, color: Colors.orange.shade300),
                      const SizedBox(height: 8),
                      Text('Upload Foto Makanan', style: TextStyle(color: Colors.orange.shade400, fontWeight: FontWeight.w500)),
                    ],
                  ),
      ),
    );
              ...
              children: [
                Expanded(
                  child: TextFormField(
                    ...
                    decoration: const InputDecoration(labelText: 'Nama Kelompok (Mis: Bumbu Halus)'),
                  ...
                  children: [
                    Expanded(
                      ...
                      child: TextFormField(
                        ...
                        decoration: const InputDecoration(labelText: 'Bahan', isDense: true),
        ...
        child: Column(
          ...
          children: [
            Row(
              ...
              children: [
                CircleAvatar(backgroundColor: Colors.orange, radius: 14, child: Text('${step.stepNumber}', style: const TextStyle(color: Colors.white, fontSize: 14))),
                IconButton(
                  icon: const Icon(Icons.delete_outline, color: Colors.red),
            ...
            TextFormField(
              ...
              maxLines: 3,
              decoration: const InputDecoration(labelText: 'Instruksi memasak', filled: true),
                      ...
                      children: [
                        Container(
                          ...
                          child: ClipRRect(
                            ...
                            child: Image.network(ApiConstants.baseUrl + step.imageUrls[imgIdx], fit: BoxFit.cover),
                          ),
                        ),
                        Positioned(
                          right: 0, top: 0,
                          child: GestureDetector(
                            onTap: () => setState(() => step.imageUrls.removeAt(imgIdx)),
                            child: Container(
  ...
  void addGroup() => setState(() => ingredientGroups.add(LocalIngredientGroup(name: '', ingredients: [LocalIngredient()])));
  void addIngredient(int groupIndex) => setState(() => ingredientGroups[groupIndex].ingredients.add(LocalIngredient()));
  void removeIngredient(int groupIndex, int ingredientIndex) => setState(() => ingredientGroups[groupIndex].ingredients.removeAt(ingredientIndex));
  void removeGroup(int groupIndex) => setState(() => ingredientGroups.removeAt(groupIndex));

  void addStep() => setState(() => steps.add(LocalStep(stepNumber: steps.length + 1, imageUrls: [])));
  ...
  Future<void> pickStepImage(LocalStep step) async {
    ...
    if (image == null) return;
    if (!mounted) return;
    final imageUrl = await context.read<RecipeProvider>().uploadStepImage(image.path);
  ...
  Future<void> submitRecipe() async {
    ...
    final payload = {
      ...
      "status": "private",
      "ingredient_groups": ingredientGroups.map((group) => {
        "name": group.name,
        "sort_order": ingredientGroups.indexOf(group),
        "ingredients": group.ingredients.map((ing) => {
          "name": ing.name, "quantity": ing.quantity, "unit": ing.unit, "sort_order": group.ingredients.indexOf(ing)
        }).toList(),
      }).toList(),
      "steps": steps.map((step) => {
        "step_number": step.stepNumber,
        "instruction": step.instruction,
        "images": step.imageUrls.map((url) => {"image_url": url, "sort_order": step.imageUrls.indexOf(url)}).toList(),
      }).toList(),
    };
    ...
    try {
      ...
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(isEdit ? 'Resep berhasil diupdate' : 'Resep berhasil dibuat')));
      Navigator.pushReplacement(context, MaterialPageRoute(builder: (_) => const MyRecipesScreen()));
    } catch (e) {
